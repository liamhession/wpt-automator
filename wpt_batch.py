...
# added the location parameter and the attempt to print it out as part of the XML filename
def SaveTestResult(output_dir, location, url, test_id, content):
  ...
  filename = '%s/%s_%s.%s.xml' % (output_dir.rstrip('/'), location, filename, test_id)
...
def RunBatch(options):
  ...
  while pending_test_ids:
    ...
    for test_id, dom in test_results.iteritems():
      SaveTestResult(options.outputdir, test_params['location'], id_url_dict[test_id], test_id,
                     dom.toxml())
    if pending_test_ids:
...
def main():
  ...
  option_parser.add_option('-s', '--server', action='store',
                           default='http://bvlabs-wpt.aws.bazaarvoice.com/',
  ...
  option_parser.add_option('-o', '--location', action='store',
                           default='bvlabs_wpt_ie9_east', help='test location')
